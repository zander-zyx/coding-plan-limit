...
**Windows**
1. 双击 `*-x64-setup.exe`（若 SmartScreen 提示，点「更多信息 → 仍要运行」）
2. 在向导中**可自行修改安装目录**（默认安装到当前用户 `%LOCALAPPDATA%\Programs` 下，无需管理员权限）
3. 点「安装」→ 完成。应用启动后静默驻留系统托盘（右下角）

**macOS**
1. 打开对应的 `.dmg`
2. **将 App 拖入「Applications」文件夹**（拖拽即安装）
3. 首次打开若提示未验证开发者：右键 App →「打开」→ 再点「打开」（或到 系统设置 → 隐私与安全性 → 仍要打开）

**Linux**
- AppImage：`chmod +x *.AppImage` 后直接运行
- deb：`sudo dpkg -i *.deb` 或双击安装

### 升级

应用内置自动检查更新（启动时 + 每 24 小时后台静默检查）：有新版本时**主界面右上角与弹窗标题栏会出现更新图标按钮**，点击直达下载页；也可以在 托盘菜单 →「检查更新」或 设置 → 关于 中手动检查。
...
| 自定义外观 | 主题（系统/浅色/深色）、主题色（预设 + 拾色器 + `#RRGGBB` 输入，弹窗实时跟随）、Logo（原色 / 单色 / 自定义图片） |
...
设置 →「外观」：主题、主题色（预设 / 拾色器 / `#RRGGBB`）、进度样式（平滑填充 / 环形）、Logo（原色 / 单色 / 自定义图片，托盘 + 标题栏 + 侧边栏同步）。悬浮窗实时跟随主题色变化。
